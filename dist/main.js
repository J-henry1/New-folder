...
let todoItems = [];
let users = [];
let sharedTodoList_Out = [];
let completed_By_User;
...
function findMatchingEmail(inputEmail, storedEmails) {
...
function validateTokenUser(token, users) {
    let parsedToken = token.split(' ')[1];
    if (jsonwebtoken_1.default.verify(parsedToken, key)) {
        let tokenEmail = jsonwebtoken_1.default.verify(parsedToken, key);
        let destructuredEmail = tokenEmail.email;
    ...
    if (username == undefined || password == undefined) {
        ...
        for (let i = 0; i < users.length; i++) {
            if (users[i].email == username) {
                if (bcrypt_1.default.compareSync(password, users[i].password)) {
                    const token = jsonwebtoken_1.default.sign({ email: users[i].email }, key);
                    console.log(token);
            ...
            if (jsonwebtoken_1.default.verify(parsedToken, key)) {
                ...
                for (let i = 0; i < users.length; i++) {
                    if (users[i].email == destructuredEmail) {
                        ...
                        let newTodo = { id: id, title: title, created_at: created_at, created_by: created_by, public_list: public_list, list_items: list_items, shared_with: shared_list };
                        todos.push(newTodo);
            ...
            if (jsonwebtoken_1.default.verify(parsedToken, key)) {
                ...
                for (let i = 0; i < todos.length; i++) {
                    if (todos[i].id == parseInt(list_id) && todos[i].created_by == created_by) {
                        ...
                        let newSharedUser = { id: token_user_id, title: todos[i].title, email: email, name: name };
        ...
        if (token) {
            ...
            for (let i = 0; i < todos.length; i++) {
                ...
                if ((todos[i].id == list_id) && (todos[i].created_by == token_user_id)) {
                    ...
                    let newTodoItem = { id: id, task: task, completed: false, completed_date: completed_date, created_at: created_at, updated_at: updated_at, list_id: list_id, due_date: due_date, completed_by_user: { email: email, name: name } };
